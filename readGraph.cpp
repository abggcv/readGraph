...
{
	Mat temp;
	resize(img, temp, Size(), 0.3, 0.3, CV_INTER_AREA);
...
void detectEdgesByHoughLines(Mat mat)
{
...
void fill_Holes(Mat image)
{
	Mat gray;
	cvtColor(image, gray, CV_BGR2GRAY);

	cv::Mat image_thresh;
	cv::threshold(gray, image_thresh, 0, 255, cv::THRESH_BINARY | CV_THRESH_OTSU);
...
void findDocumentEdges(Mat img)
{
...
void findGraphValues(Mat graph)
{
	...
	Mat thresh;
	threshold(graph.clone(), thresh, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);

	showReducedImage("threshold", thresh);

	findContours( graph.clone(), cnts, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );

	//draw contours
	Mat cntImg = Mat::zeros(graph.size(), CV_8UC1);
	
	for(int i = 0; i < cnts.size(); i++)
		drawContours(cntImg, cnts, i, Scalar(255));
...
vector<float> findUniqueValues(const Mat& input, bool sort = false)
{
...
bool findKMeansClusters(Mat img, int k)
{
	vector<Mat> imgRGB;
    split(img,imgRGB);
